...
namespace WindowsFormsAppAdoNet
{
    public class KategoriDAL:OrtakDAL
    {
        public int Add(Category category)
        {
            int sonuc = 0;
            ConnectionKontrol();
            SqlCommand command = new SqlCommand("Insert into Kategoriler(Name,Description,CreateDate,Durum) values (@Name,@Description ,@CreateDate,@Durum)", _connection);
            command.Parameters.AddWithValue("@Name", category.Name);//güvenlik açığından dolayı parametre kullanılır
        ...
        public int Update(Category category)
        {
            int sonuc = 0;
            ConnectionKontrol();
            SqlCommand command = new SqlCommand("Update Kategoriler set Name=@Name,Description=@Description,CreateDate=@CreateDate,Durum=@Durum where Id=@Katİd", _connection);
            command.Parameters.AddWithValue("@Name", category.Name);//güvenlik açığından dolayı parametre kullanılır
            command.Parameters.AddWithValue("@Description", category.Description);
            command.Parameters.AddWithValue("@CreateDate", category.CreateDate);
            command.Parameters.AddWithValue("@Durum", category.Durum);
            command.Parameters.AddWithValue("@Katİd", category.Id);
            sonuc = command.ExecuteNonQuery();
